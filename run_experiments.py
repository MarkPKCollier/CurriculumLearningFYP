...
--region=us-central1 \
--job-dir=gs://mcollier1002/{0}_mann_{1}_curric_{2}_pad_{3}_num_{6}_{7} \
--runtime-version=1.2 \
    ...
    for mann, num_units, num_layers in manns:
        for curriculum in curricula:
            for pad_to_max_seq_len in [False]:
                for init_mode in ['constant', 'random']:
                    os.system(
                        run_cmd.format(
                            task, mann, curriculum, pad_to_max_seq_len, num_units, num_layers, VERSION, experiment_number+1, max_seq_len, num_bits_per_vector, init_mode))
