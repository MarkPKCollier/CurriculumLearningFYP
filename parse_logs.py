...
for log_file in glob.glob('logs/*.txt'):
    ...
    res = []
    res.append((log_file + '_step', log_file + '_curriculum', log_file + '_loss', log_file + '_bit_errors'))
    for i, curricilum, loss, bit_errors in re.findall('TRAIN_PARSABLE: (\d+),(\d+),(-?\ *[0-9]+\.?[0-9]*(?:[Ee]\ *-?\ *[0-9]+)?),([-+]?\d*\.\d+|\d+)\n', f):
